...
const sendInvoiceEmail = async (email, userName, purchaseAmount,paymentMethod,orderItems) => {
    try {
        ...
        const mailOptions = {
            from: "atindramohandas353@gmail.com", // Replace with your email address
            to: email,
            ...
            html: `
                <div style="background-color: #f2f2f2; padding: 20px;">
                    ...
                    <p style="color: #333; font-size: 16px;">Purchase Amount: $${purchaseAmount}</p>
                    <p style="color: #333; font-size: 16px;">Payment Method: ${paymentMethod}</p>
                    <p style="color: #333; font-size: 16px;">Order Items:</p>
                    <ul>
                        ${orderItems.map(item => `<li style="color: #333; font-size: 16px;">${item.name} - $${item.price}</li>`).join('')}
                    </ul>
